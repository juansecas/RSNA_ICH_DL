...
class DataGenerator(keras.utils.Sequence):
    ...
    def on_epoch_end(self):
        ...
        if self.labels is not None: # for training phase we undersample and shuffle
            ...
            keep_prob = self.labels.iloc[:, 0].map({0: 0.8, 1: 1})
...
#             
    ...
    def fit_and_predict(self, train_df, test_df):
        ...
        # callbacks
        pred_history = PredictionCheckpoint(test_df, input_size=self.input_dims)
...
for train_index, test_index in kf.split(df.index, df):
    ...
    model = MyDeepModel(engine=ResNet50, input_dims=(224, 224, 3), batch_size=32, learning_rate=5e-4, 
                        num_epochs=5, decay_rate=0.8, decay_steps=1, weights="ModelV5_Weights.h5", verbose=1)
    
    # obtain test + validation predictions (history.test_predictions, history.valid_predictions)
    
    #history = model.fit_and_predict(df.iloc[train_idx], df.iloc[valid_idx], df.iloc[test_idx])
    
    history = model.fit_and_predict(df.iloc[train_index], df.iloc[test_index])

    pred_df = df.iloc[test_index].reset_index(col_level=1)
    ...
    for e in range (0,6):
        ...
        TEST_ACCURACY.append(accuracy_score(y_test_sample, y_pred_sample))
        TEST_RECALL.append(recall_score(y_test_sample, y_pred_sample))
        TEST_F1.append(f1_score(y_test_sample, y_pred_sample))
    
    metrics = [TEST_ACCURACY, TEST_RECALL ,TEST_F1]
    ...
    metrics_df.to_csv('resultados/metrics-Cross-{}.csv'.format(fold_var))
