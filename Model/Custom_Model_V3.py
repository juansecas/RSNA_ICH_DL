...
class DataGenerator(keras.utils.Sequence):
    ...
    def on_epoch_end(self):
        ...
        if self.labels is not None: # for training phase we undersample and shuffle
            ...
            keep_prob = self.labels.iloc[:, 0].map({0: 0.5, 1: 1})
...
class MyDeepModel:
    ...
    def _build(self):
        ...
        self,model = EfficientNetB0(include_top=False, input_shape=(*self.input_dims, 1),
                                    classes=6)
...
model = MyDeepModel(engine=ResNet50, input_dims=(224, 224), batch_size=32, learning_rate=1e-3, 
                    num_epochs=10, decay_rate=1, decay_steps=1, weights="imagenet", verbose=2)
